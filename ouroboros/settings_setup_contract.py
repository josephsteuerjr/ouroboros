...
_MODEL_DEFAULTS = {
    ...
    "qwen": {key: value for key, value in QWEN_DIRECT_DEFAULTS.items() if key != "heavy"},
    "kimi": {key: value for key, value in KIMI_DIRECT_DEFAULTS.items() if key != "heavy"},
...
_PROVIDER_FIELDS = _rows(("id", "stateKey", "settingKey", "settingsInputId", "label", "placeholder", "note", "inputType", "group"), (
    ...
    ("qwen-key", "qwenKey", "DASHSCOPE_API_KEY", "s-qwen-key", "Alibaba DashScope API Key (Qwen)", "sk-...", "Optional. If this is the only remote key, the next step prefills Qwen's own model ids.", "password", "more"),
    ("kimi-key", "kimiKey", "MOONSHOT_API_KEY", "s-kimi-key", "Moonshot API Key (Kimi)", "sk-...", "Optional. If this is the only remote key, the next step prefills Kimi's own model ids.", "password", "more"),
...
_PROFILE_SPECS = {
    ...
    "qwen": ("Alibaba DashScope (Qwen)", "DashScope is present, so the next step prefills Qwen's own model ids.", "DashScope-only setup detected. These defaults use qwen3-max for main work and qwen3-flash for the light lane."),
    "kimi": ("Moonshot (Kimi)", "Moonshot is present, so the next step prefills Kimi's own model ids.", "Moonshot-only setup detected. These defaults use kimi-k2-turbo-preview for main work and moonshot-v1-8k for the light lane."),
