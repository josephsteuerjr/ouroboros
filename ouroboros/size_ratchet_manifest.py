...
BAND_PATHS = {
    ...
    "ouroboros/llm_claudexor.py": "Entered the band at 1004 only on the merge of upstream's OpenAI-family cache layout (declared-prefix projection in _request, wire_layout disclosure in finish) with the Presence quota re-ask and all-operations-not-started proof in chat_claudexor: disjoint hunks of one caller-owned transport, each side alone at 1000; one owner, shrink-only from here.",
