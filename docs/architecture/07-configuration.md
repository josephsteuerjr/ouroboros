...
| ZAI_API_KEY | "" | Z.ai (GLM) direct key (`zai::...` values) |
| ZAI_PLAN | "" | Z.ai plan: `payg` (default, pay-as-you-go) or `coding` (Coding Plan endpoint) |
| DASHSCOPE_API_KEY | "" | DashScope (Qwen) direct key (`qwen::...` values; region-bound) |
| MOONSHOT_API_KEY | "" | Moonshot (Kimi) direct key (`kimi::...` values) |
...
Z.ai (`zai::`): one OpenAI-compatible API surface on two plans sharing the same key — `ZAI_PLAN=coding` selects the subscription Coding Plan endpoint (`api.z.ai/api/coding/paas/v4`, officially intended for supported coding tools only), the default `payg` uses pay-as-you-go (`api.z.ai/api/paas/v4`); `provider_models.resolve_zai_base_url` is the one resolver. Slash-form `zai/...` stays OpenRouter. The provider has exactly three reasoning tiers (`low | high | max`; `medium` does not exist), an absent `reasoning_effort` is served at MAX, and thinking cannot be disabled — `thinking={"type":"disabled"}` answers 400 code 1210 ("please use low, high or max") on PAYG. The canonical effort scale is therefore projected onto Z.ai's own wire dialect at the send boundary (none/minimal/low -> low, medium/high -> high, xhigh/ultra -> max; projection table: `provider_models.ZAI_REASONING_EFFORT_ALIASES`) and every tier change is disclosed as `reasoning_effort_clamped`. Unlike DeepSeek there is no forced-tool exception: `tool_choice=required` and named functions work with thinking enabled on the coding endpoint, so a forced tool call keeps its effort tier. A Coding Plan key on the default `payg` plan answers 429 code 1113 ("Insufficient balance") — the provider Test and Settings surfaces surface that message with a plan hint rather than a bare "Rate limited".

DashScope (`qwen::`): the international OpenAI-compatible endpoint is the fixed constant `provider_models.DASHSCOPE_BASE_URL`; region-bound keys (an international key cannot call the China endpoint and vice versa), `enable_thinking` toggles via `extra_body` stay out of scope — the generic canonical request is sent as-is, and the canonical reasoning-effort tier does NOT reach this route yet: DashScope's thinking parameters differ and no live measurement has been made, so effort is silently dropped here rather than guessed onto the wire. A proxy or mirror belongs to `openai-compatible::`; slash-form `qwen/...` stays OpenRouter (and names a real OpenRouter vendor namespace for the vision families).

Moonshot (`kimi::`): one official OpenAI-compatible endpoint (`provider_models.MOONSHOT_BASE_URL`). Named `tool_choice` combined with thinking is a documented provider 400 — forced tool selection is served without thinking or refused per provider contract; otherwise the canonical request is sent as-is. As with `qwen::`, the canonical reasoning-effort tier does NOT reach this route: Moonshot's thinking parameters differ and no live measurement has been made. Slash-form `kimi/...` stays OpenRouter.
